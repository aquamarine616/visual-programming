...
function getNumber(cells: Cells, ref: string): number {
  const n = Number(getValue(cells, ref))
  return isNaN(n) ? 0 : n
}
...
function evalExpr(cells: Cells, expr: string): number | string {
  let s = expr.trim()
  ...
  if (fn) {
    ...
    if (name === 'SUM') {
      let sum = 0
      for (const r of refs) sum += getNumber(cells, r)
      return sum
    }
    if (name === 'AVERAGE') {
  ...
  s = s.replace(/[A-Z]+\d+/g, (ref) => String(getNumber(cells, ref)))
...
export function getValue(cells: Cells, ref: string, seen: Set<string> = new Set()): string {
  ...
  seen.add(ref)
  return String(evalExpr(cells, raw.slice(1)))
}